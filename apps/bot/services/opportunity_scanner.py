...
4. Score: Calculate contrarian opportunity score for each
5. Select: Top 5-10 assets with score >= 40
...
class OpportunityScanner:
    """
    Scans Kraken for contrarian trading opportunities.
    ...
    async def run_scan(self) -> ScanResult:
        ...
        logger.info(f"\n{'='*60}")
        logger.info(f"[Scanner] Starting opportunity scan at {start_time.isoformat()}")
        logger.info(f"{'='*60}")
        ...
        logger.info(f"[Scanner] Selected top {len(candidates)} candidates for scoring")

        # Phase 3: Fetch OHLCV and score each candidate
        logger.info("[Scanner] Phase 3: Fetching OHLCV and scoring...")
        ...
        for i, (symbol, ticker_data) in enumerate(candidates):
            try:
                ...
                # Calculate score
                score = calculate_contrarian_score(symbol, candle_dicts, ticker_data)
                scores.append(score)

                if score.total_score >= self.config.min_score:
                    logger.debug(
                        f"[Scanner] {symbol}: Score {score.total_score:.1f} - {score.reasoning}"
                    )
                ...
                if (i + 1) % 10 == 0:
                    logger.info(f"[Scanner] Scored {i + 1}/{len(candidates)} candidates...")
        ...
        logger.info("[Scanner] Phase 4: Ranking opportunities...")
        ...
        logger.info(f"[Scanner] - Pairs scored: {len(scores)}")
        logger.info(f"[Scanner] - Opportunities found: {len(top_opportunities)}")
        ...
        if top_opportunities:
            ...
            for i, opp in enumerate(top_opportunities, 1):
                logger.info(
                    f"[Scanner]   {i}. {opp.symbol}: "
                    f"Score {opp.total_score:.1f} "
                    f"(Vol ${opp.volume_24h_usd/1e6:.1f}M) - {opp.reasoning}"
                )
